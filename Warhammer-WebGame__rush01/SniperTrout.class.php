...
class SniperTrout extends Ship {
	...
	public function fight(array $kwargs) {
		$ship = $this->type($kwargs['dice_roll'], $kwargs['width'], $kwargs['height'],
			...
			$kwargs['direction']);
		if ($ship === NULL)
			error_log("ship that was shot was null (fight function, SniperTrout.class.php)");
		else {
...
{
	return file_get_contents('./SniperTrout.doc.txt') . PHP_EOL;
}
